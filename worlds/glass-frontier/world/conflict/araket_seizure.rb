conflict :araket_seizure do
  ...
  summary "The Araket Seizure is the ongoing armed occupation of Anaret's lower freight approach, where raiders hold captive cutters. Sira Dhen tends the wounded while outside crews prepare extraction and attack routes."
  subkind :dispute
  status :complete
  ...
  log "2026-09-21 — Restored Sira Dhen's name within the occupation account and made her treatment shelter's immediate choices explicit. Her biography remains here rather than acquiring a separate NPC article."
  prose <<~PROSE
  ...
  prose <<~PROSE, section: :tensions, heading: "Getting People Out"
    ...
    #{ref :tovin, "Tovin"} will carry injured people aboard #{ref :ulveth, "Ulveth"}. He wants the approach watched while he lands and enough ground crew to load quickly. Sira will go with her patients, including injured occupiers if they surrender their weapons. Some captives object to sharing their return ship with the people who confined them. She has put a raider's confiscated pistol beneath a water case and asked its former owner's least injured captive to keep it there. Both can see the weapon while she works.

    Sira charges expeditions for field work and treats ordinary patients near Ressa's hall between journeys. An employer's injured rival can occupy her cradle; she states that before accepting payment. Here she carries the cradle when there are too few hands and insists on testing the route with a patient's weight. She has refused to call an armed expedition a medical escort merely because its commander lends her a vehicle. The shelter's doorway is marked with the width of the loaded cradle. A volunteer who can slip through a cut alone must still bring back a route wide enough for that mark.
  ...
  prose <<~PROSE, section: :present_day, heading: "Who Holds the Entrance Afterward"
    ...
    #{ref :nereth_valis, "Nereth Valis"} wants the captives freed and the retained witnesses left where they can be compared. #{ref :eris_talven, "Eris Talven"} continues preparing the larger return outside the occupied rooms. The rescuers have marked their immediate approaches on a drawing of the broken district: the service cover, Eneth's surviving bands and the cabin's exposed rear fittings. Sira has crossed out one proposed turn where a carried patient would have to be set upright.
