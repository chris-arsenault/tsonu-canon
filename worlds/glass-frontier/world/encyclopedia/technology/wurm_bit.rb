encyclopedia :wurm_bit do
  name "Wurm-bit"
  summary "A wurm-bit is a hand- or cutter-mounted boring machine built around a shed shearwurm mouth plate. Salvagers use its changing bite angle to follow curved seams through mixed mineral and ringglass."
  kind :technology
  subkind :machine
  function "Cuts curved mineral and ringglass seams with a mounted shearwurm mouth plate"
  status :complete
  log "2026-09-23 — Replaced a cue and both variations that repeated the prose with a fresh sound cue and two genuine operating variations."
  ...
  descriptive_identity(
    appearance:
      "A cleaned shearwurm mouth plate seated in a forked yoke, dark " \
    ...
    risks:
      ...
      "boundaries. The repeated bite copies part of a shearwurm's " \
  ...
  prose <<~PROSE
    ...
    The result is a curved, slightly ribbed cut like the wall of a shearwurm tunnel. Salvagers use wurm-bits to free intact material, open wreck panels without crossing tuned members, and make receiving holes that preserve the load around them.
  PROSE

  prose <<~PROSE, section: :structure, heading: "Plate, Yoke, and Feed"
    A cleaned mouth plate sits in a forked yoke with its dark root against a weak ringglass driver. The driver supplies a broad structural note while a mechanical feed presses the edge into the work. The plate twists itself toward the clearest boundary between materials.
  ...
  prose <<~PROSE, section: :dangers, heading: "Calling Through the Work"
    A wurm-bit copies part of the feeding pattern that shaped its plate. Active shearwurms can feel the repeated bite through a connected debris mass. Small animals investigate from existing tunnels. Large ones may cut a new path toward the tool.
  ...
  cue "A working bit gives a stuttering grind that climbs and drops as the plate twists, and the operator's hands drift left and right on the frame like someone steering a cart downhill."
  ...
  pressure "The plate folds flat against the yoke the same way when the seam ends, divides, or turns harder than the mount allows, and a crew paid by the tonne is tempted to press on and learn which. Pressing the feed anyway breaks the plate into pieces that then run off along different boundaries."
  variation "Crews in shearwurm country run two plates on a slow alternating driver so the bite never repeats long enough to call; crews in fragments the wurms have abandoned run one plate flat out and finish in half the time."
  variation "Old operators steer by the push of the yoke in their palms and cut the witness wires off as clutter; yards that rent bits refuse them, and they buy their own."
