geographic_location :anaret do
  ...
  prose <<~PROSE, section: :people, heading: "The Borrowed Enclosure"
    The treatment shelter has a shared #{encyclopedia_ref :esken} with one dented side petal. Its keepers have tested the closure and marked the narrowest turn on the way to Ysil's cut. A person inside can survive the exposed crossing, but the enclosure must be tilted through that turn. Companions practice with ballast while people waiting for treatment watch from the canopy.
  PROSE
  prose <<~PROSE, section: :present_day, heading: "The Recoverable District"
    A composite return, the #{ref :great_restorer, "Great Restorer"}'s working, makes one moment of a broken structure's past whole again out of its scattered surviving pieces, and brings back everything that was true of it at that moment. #{ref :eris_talven, "Eris Talven"} and #{ref :nereth_valis, "Nereth Valis"} have matched enough pieces to describe one such moment in Anaret, when the freight descent, Eneth's deck and the adjoining workshops all stood together. Surviving foundations fix their positions. Retained fittings show that at that moment Eneth's saddle was already worn and Lethai's optical mount already cracked. A return would bring both back exactly so.
    ...
    Eris still has two choices to make: how far the working reaches, and which one part of it he will name beforehand and leave unrecovered. Every great return leaves such a part behind, and its absence enters the restorer's own body. The rooms beyond the compared ones sit outside the working as it is drawn today. Each newly reached room can add a matching piece, support a separate restoration or turn out to be built another way entirely, and the diggers at the district's far edge keep finding more wall.
  PROSE
  prose <<~PROSE, section: :tensions, heading: "People Within the Same Walls"
    The bench crew keeps its successful optical piece in a shelter of its own, and the crew has split over it. Dova Rusk, who ground the curve, wants it sold to pay for another season's digging; she has shown it through the window to two buyers already. Her bench partner Ennick Sall has promised it to the people trying to reach the captives, on the grounds that a curve which hides a hand could hide a hand on a latch. Independent arrivals bargain with whichever of them answers the door.

    #{ref :ysil, "Ysil"}'s climbers leave spare anchors along the upper route where a following party can reach them, because Ysil means the route to outlast both armed crews. #{ref :selven_iral, "Selven Iral"}'s advance party has started asking which of those anchors would take the weight of a shield on the way across the court. Selven means to clear the occupiers and keep the district. Araket's captain means to leave Lithren with one load worth the trip. Sira Dhen means to leave with every injured and captive worker, carried out if they cannot climb.

    Research goes on inside the siege. #{ref :tamet, "Tamet"} and the comparative expedition's materials investigators compare the workshop fittings with discoveries at #{ref :damarat, "Damarat"}, and their trial pieces sit beside machines they can reach only by the upper cut. One crew has packed its successful samples inside meal tins from #{ref :ressa_dorr, "Ressa Dorr"}, ready to go out with an injured companion. Another has left a camera looking past Sovan's turning track; its lead runs under a sleeping platform, and whoever lies there listens through the boards for the carriage coming back while an armed watch paces the lower court.

    Below them, the occupiers have connected a cutting branch to a steering mirror. A bright spot now travels across the doorway their guns cannot be raised far enough to cover, and the shelter's occupants have learned to step around it.
  PROSE
