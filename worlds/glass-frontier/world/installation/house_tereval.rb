installation :house_tereval do
  ...
  prose <<~PROSE, section: :present_day, heading: "What Is Being Offered"
    ...
    The #{ref :deral_hunters, "Deral Hunters"} have visited to discuss an offer for living eggs. Mareva is willing to put a workshop vessel at their disposal for the rescue if the buyer pays for its voyage. She had transparent cases brought down for inspection; the hunters pressed a hooked tool against the thin lid until its hinge bent. The cases remain on the terrace while the buyer argues for thicker glass and the hunters ask who will feed what hatches.

    One private examination has brought an original #{ref :talessar, "Talessar"} leaf into a room above the pools under a Serevan escort. Its resistance keeper has promised to take it away afterward. Mareva allowed the examination for guests disputing the street copies; league supporters have since proposed a later exhibition at which Aren would accept a crown. He has received their invitation and has yet to accept. Ilessa has moved the case away from the bridge window after finding a guest trying to photograph one face through the glass.

    A guest invited to that examination is also being courted as a possible successor. The #{ref :severance, "Severance"} has sent an offer of passage away; other #{ref :velisar_resistance, "resistance"} members want the guest brought to their rescuers. Ilessa keeps both messages in the wardrobe until the guest can read them alone. Across the pool, Mareva has seated a captive's family beside the officer whose promise could release their relative from #{ref :savren, "Savren"}. The officer wants her backing for the crown exhibition before giving that promise.
  PROSE
